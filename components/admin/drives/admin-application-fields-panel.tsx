...
export function AdminApplicationFieldsPanel({
  fields = [],
  onChange,
}: AdminApplicationFieldsPanelProps) {
  ...
  const availableToAdd = AVAILABLE_STUDENT_FIELDS.filter((f) => {
    ...
    if (catalogCategory !== "All" && f.category !== catalogCategory)
      return false;
  ...
  function handleAddField(catalogItem: ApplicationFieldDef) {
  ...
  function handleRemoveField(keyToRemove: string) {
    onChange?.(activeFields.filter((f) => f.key !== keyToRemove));
  }

  function handleToggleRequired(keyToToggle: string) {
    onChange?.(
      activeFields.map((f) => {
  ...
  function applyPreset(presetKey: string) {
    ...
    const newFields: ApplicationFieldConfig[] = preset.keys.map((key) => {
  ...
  function handleAddCustomField(e: React.FormEvent) {
    ...
    const newField: ApplicationFieldConfig = {
      ...
      icon: "📝",
      description:
        customField.description.trim() ||
        "Custom information requested by placement admin",
    ...
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          >
            {activeFields.length} Fields Configured ({mandatoryCount} Mandatory,{" "}
      ...
      >
        ...
        <span>
          <strong>Profile Integration:</strong> All selected fields (e.g. Full
          Name, Roll No, CGPA, Technical Skills, GitHub, etc.) are verified and
          pre-populated directly from the student&apos;s profile records when they
          click <em>Apply Now</em>.
        </span>
      </div>
          ...
          {Object.entries(FIELD_PRESETS).map(([key, p]) => (
            <button
              ...
              className={`btn btn-sm ${
                selectedPreset === key ? "btn-primary" : "btn-outline"
              }`}
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {categories.map((c) => (
                <button
                  ...
                  className={`btn btn-sm ${
                    catalogCategory === c ? "btn-primary" : "btn-ghost"
                  }`}
      ...
      {activeFields.length === 0 ? (
        ...
        >
          ...
          <div style={{ fontWeight: 600, marginBottom: 4 }}>
            No Application Fields Configured
          </div>
          <div
            style={{
              fontSize: 12,
          ...
          >
            Click one of the Quick Presets above (e.g. Standard Core) or add
            fields from the profile catalog to specify what students must
            provide.
          </div>
          <button
            type="button"
          ...
          <table>
            ...
            <tbody>
              {activeFields.map((field, index) => {
                const catalogItem = AVAILABLE_STUDENT_FIELDS.find(
                  (f) => f.key === field.key
                );
                  ...
                  <tr key={field.key || index}>
                    ...
                    <td>
                      <button
                        ...
                        className={`btn btn-sm ${
                          isMandatory ? "btn-primary" : "btn-outline"
                        }`}
                        style={{
                          fontSize: 11,
                          padding: "3px 10px",
      ...
      <Dialog open={showCustomModal} onOpenChange={setShowCustomModal}>
        <DialogContent style={{ maxWidth: 440 }}>
          <DialogHeader>
            <DialogTitle>Add Custom Application Field</DialogTitle>
          </DialogHeader>
          <p
            className="text-secondary"
            style={{ fontSize: 12, margin: "6px 0 16px" }}
          >
          ...
          <form onSubmit={handleAddCustomField}>
            <div className="field" style={{ marginBottom: 12 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCustomField((c) => ({ ...c, label: e.target.value }))
                }
              />
            </div>
