...
- **File Storage (Vercel Blob)**: binary/large content only — student profile photos and drive job-description PDFs, both persistent. Uploaded Excel/CSV files for bulk student import are also written to Blob, but only *transiently*: the commit re-reads the file from Blob (only the admin's own upload — `isOwnImportFileUrl`) and deletes it as soon as the clean rows are committed. Rows held back for errors are not kept in Blob: they travel back to the admin as the error review, exportable as an error sheet. A commit that fails outright leaves the file for a retry.
- The database stores a URL/reference to any blob it needs to reference (profile photo, JD PDF) — never the file itself.
- **Student identity.** `Student.id` (cuid) stays the primary key — every foreign key points at it. The institutional identifier is `misNumber` (unique), with `prnNumber` (unique, optional), `rollNumber` (unique) and `email` (unique). All four are stored normalised by `features/students/utils/student-identity.ts` (upper case, no whitespace; email lower case), and the MIS/PRN shape is also a CHECK. MIS, roll number and batch are required at every write boundary (import, manual add, verification, approval); the columns stay nullable only for students created before Phase 1.
- **Batch is the expected passout year.** `Student.expectedPassoutYear` (e.g. 2027) is the stored batch; the label ("2023-27") is derived by `batchLabel` in `features/students/utils/batch.ts` and nowhere else, and a typed batch is read by `parseBatch` there. It replaced `Student.batchYear`, whose meaning was never defined and which was dropped, not copied. Every batch filter, BATCH_YEAR drive rule and announcement batch target is a list of passout years.
...
- Students reach the platform two ways, and both converge on the same `Student` record:
  ...
  2. **Self sign-up.** Anyone can sign up through Clerk, but signing up does not grant student access. The student fills the verification card — MIS, name, phone, roll number, department and batch required, PRN optional; the email is the Clerk-verified one and is never typed. The MIS number finds the roster record; name, roll number, department and batch must all agree with it; and the record's email must be the account's verified email. Then the account is linked (`isPending → false`) and admitted immediately. No record for that MIS or email creates a `StudentAccessRequest` and a "waiting for confirmation" screen.
- The rules live in one pure function, `decideRegistrationOutcome` (`features/students/utils/registration-match.ts`). A partial match — right MIS, wrong roll number — is refused with one message that never names the wrong field, so nobody can probe a stranger's record field by field. When every detail matches but the roster lists a different email, only a department admin can vouch: the request carries the MIS number, and approving it links the account to that roster record (taking the verified email) instead of creating a second one. A roster row already linked to another account is refused outright rather than re-linked.
...
- **Department (its own admins):** venue, reporting time, coordinator, contact
  ...
  default batch (`defaultPassoutYear`) prefilled when adding a student. The department comes from the
...
13. There is one "Placement Rate": placed ÷ the *eligible pool*, defined once as `eligiblePoolSql` (registered, and either opted in or already placed) and computed with `percentOfPool`, both in `placement-status.ts`. The department Overview (`getAdminDashboardStats`), department Insights (`getDepartmentInsights`), and the Super Admin's matrix and system stats (`getDepartmentMatrix`, `getSystemStats`) all use it; `placement-rate.test.ts` feeds the same counts to all four and requires one answer. The numerator is "placed AND in the pool" — an off-campus placement can be recorded for an unregistered student, and the rate must never exceed 100%. Participation Rate is applied ÷ the same pool. Never compute a rate inline against `totalStudents` or `registeredStudents` (that is how three definitions drifted apart before Phase 10). The three-category placement-status vocabulary a Super Admin or department admin filters by (Placed / Not placed / Not yet eligible) lives once, as named Prisma filters in `placement-status.ts` (`NOT_PLACED_STUDENT_FILTER`, `NOT_YET_ELIGIBLE_STUDENT_FILTER`) — a screen that needs this grouping imports the filter, it does not re-derive `isPending`/`optedIn`/placement logic inline.
14. Filtering students by placement status uses `PLACEMENT_STATE_FILTERS` (one Prisma filter per `PlacementState`, in `placement-status.ts`), which mirrors `resolvePlacementState`'s precedence — pending, then placed, then opted out, else eligible — so every student matches exactly one filter, the one their badge shows. Never filter "placed" with bare `PLACED_STUDENT_FILTER` or "opted out" with bare `optedIn: false` on a roster: the first catches a placement recorded before registration, the second a student placed and then opted out. `tests/integration/student-directory.integration.test.ts` checks the partition against real rows.
15. A dialog or panel built for the department admin who owns the record (recording a placement, dropping a student) is reused for the Super Admin's read-only oversight via an explicit `readOnly`/`canManage` prop that hides the mutating controls — never a second component. The underlying read query widens from `requireDepartmentAdmin` to `requireAnyRole(["DEPT_ADMIN", "SUPER_ADMIN"])` (department admin keeps its own "not found and not yours read the same" check; the Super Admin skips it), while every mutating action keeps its original, narrower authorization untouched. See `StudentDetailsDialog`'s `readOnly` prop and the Super Admin's institution-wide directory (Phase 9, Item 21). 